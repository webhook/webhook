...
function firebaseReserverSite(config, step) {
  firebaseRoot.auth(config.firebaseToken, function(error, auth) {
    ...
    firebaseRoot.root().child('has_whitelist').once('value', function(hasWhitelistSnap) {
      var hasWhitelist = hasWhitelistSnap.val();
      firebaseRoot.root().child('whitelist').child(config.webhookEscapedUsername).once('value', function(snap) {
        var val = snap.val();
        ...
        if(!val && hasWhitelist) {
          ...
          console.log('# can create new sites.'.red);
          console.log('# ---------------------------------------------------- #'.red);
          process.exit(1);
        } else { 
            ...
            if(err.code === 'PERMISSION_DENIED') // Either this doesn't exist yet or it does and we dont have access
            {
              firebaseRoot.child('sites/'+ config.siteName + '/owners').set(data, function(err, data) {
                if(err)
                {
                  ...
                  console.log('# You don\'t have permission'.red);
                  console.log('========================================================'.red);
                  console.log('# This site name has already been taken by another user.'.red);
                  console.log('# Please try a different one.'.red);
                  console.log('# ---------------------------------------------------- #'.red);
                  process.exit(1);
                }
